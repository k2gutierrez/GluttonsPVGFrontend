...
export function Header({ isolated = false }: { isolated?: boolean }) {
  ...
  const links = stage === 'mint'
    ...
    : stage === 'live'
      ? [{ href: '/', label: 'LIVE' }, { href: '/my-gluttons', label: 'MY GLUTTONS' }, { href: '/leaderboard', label: p.isSettled ? 'FINAL TABLE' : 'LEADERBOARD' }, { href: '/rules', label: 'RULES' }]
      : [];
  ...
  return <header className="site-header">
    <div className="header-inner">
      ...
      <div className="header-actions ml-auto flex items-center gap-3">
        ...
        {stage !== 'awareness' && stage !== 'syncing' && <button className="mobile-menu-toggle" aria-expanded={menuOpen} aria-controls="mobile-nav" onClick={() => setMenuOpen(v => !v)}>{menuOpen ? 'CLOSE' : 'MENU'}</button>}
      </div>
    </div>
