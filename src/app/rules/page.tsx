...
export default function Rules(){
  ...
  return <><Header/><main className="page-shell rules-page">
    ...
    <Panel className="mt-5 p-5 md:p-7"><Kicker>how the game escalates</Kicker><div className="bar phase-bar"><i style={{width:'74%'}}/></div><div className="mt-6 grid gap-3 md:grid-cols-3"><Phase n="01" name="THE FEAST" trigger="GAME START" actions="FEED / FAST / POISON / FRESH / ROTTEN / KEEP FRESH"/><Phase n="02" name="THE PLAGUE" trigger="ALIVE ≤ 50% OR MEAL = +2H" actions="ALL FEAST ACTIONS + DEVOUR THE LIVING"/><Phase n="03" name="LAST SUPPER" trigger="ALIVE ≤2.5% OR DAY 120 + 1H WARNING" actions="FRESH / ROTTEN / DEVOUR / TRUCE"/></div></Panel>
    <Panel className="mt-5 p-5 md:p-7"><Kicker>poison / final lock</Kicker><div className="grid gap-3 md:grid-cols-3"><div className="phase-card"><b>ATTACKER</b><p>&gt;1H · -1H on success · NO COOLDOWN.</p></div><div className="phase-card"><b>NORMAL TARGET</b><p>&gt;1H · clock halves with 1H floor · SHIELD UP for 10H.</p></div><div className="phase-card"><b>FASTING TARGET</b><p>Poison triggers FINAL BITE 1H. Rescue before zero or die.</p></div></div></Panel>
    <Panel className="mt-5 p-5"><Kicker>money moves the same machine</Kicker><div className="grid gap-4 md:grid-cols-3 xl:grid-cols-6"><Money a="COMMUNITY MINT" b={`${formatEther(p.communityMintPrice)} ${NATIVE_SYMBOL}`} c="DYNAMIC / 95% POT"/><Money a="PUBLIC MINT" b={`0.004 ${NATIVE_SYMBOL}`} c="95% POT / 5% PVG"/><Money a="FEED" b={`0.0006 ${NATIVE_SYMBOL}`} c="92% POT / 8% PVG"/><Money a="POISON" b={`0.0004 ${NATIVE_SYMBOL}`} c="92% POT / 8% PVG"/><Money a="KEEP FRESH" b={`0.0003 ${NATIVE_SYMBOL}`} c="24H FRIDGE / 92% POT"/><Money a="ROYALTY" b="4%" c="7/8 PLAYER / 1/8 PVG"/></div></Panel>
