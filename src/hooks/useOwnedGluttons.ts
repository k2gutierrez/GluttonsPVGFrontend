...
export function useOwnedGluttons(){
 ...
 const artFails=useRef<Map<number,number>>(new Map());
 // Metadata is cosmetic and may never block gameplay state. A whale wallet hydrates only six
 // images at a time so opening My Gluttons cannot create an API/RPC burst. Failures are parked
 // for a minute so a dead endpoint cannot become a per-second retry storm.
 useEffect(()=>{let dead=false;const nowMs=Date.now();const targets=visibleRaw.filter(t=>!t.burned&&t.owner).map(t=>({t,vs:derivedVsOf(t)})).filter(x=>images[x.t.id]?.vs!==x.vs&&nowMs-(artFails.current.get(x.t.id)||0)>60_000).slice(0,6);if(!targets.length)return;void Promise.all(targets.map(async({t,vs})=>{try{const r=await fetch(`/api/read/metadata/${t.id}?b=${t.updatedBlock||0}`,{cache:'no-store'});if(!r.ok)throw new Error(`HTTP ${r.status}`);const j=await r.json();const img=String(j.image||'')||await metadataImage(String(j.tokenUri||''),vs);if(!img)throw new Error('NO_IMAGE');artFails.current.delete(t.id);return{id:t.id,src:img,vs}}catch{artFails.current.set(t.id,Date.now());return null}})).then(rows=>{if(dead)return;const clean=rows.filter(Boolean) as {id:number;src:string;vs:number}[];if(!clean.length)return;setImages(x=>{const y={...x};for(const c of clean)y[c.id]={src:c.src,vs:c.vs};return y})});return()=>{dead=true}},[visibleRaw,images,derivedVsOf]);
