...
export async function GET(req:Request,{params}:{params:Promise<{address:string}>}){try{
 ...
 const rows=await rpcExecute('mint wallet snapshot',()=>serverClient.multicall({allowFailure:true,deployless:true,batchSize:0,contracts}));if(rows.some((x:any)=>x.status!=='success'))return NextResponse.json({error:'MINT_WALLET_SNAPSHOT_INCOMPLETE',retryable:true},{status:503,headers:{'Cache-Control':'no-store'}});
