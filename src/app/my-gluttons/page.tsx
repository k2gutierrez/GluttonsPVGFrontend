...
import { ASSETS, CONTRACTS, FEED_PRICE, GAME_ENGINE_ABI, GAME_HOUR_SECONDS, GLUTTON_NFT_ABI, NATIVE_SYMBOL, POISON_PRICE, POWER_PRICE } from '@/lib/constants';
...
export default function MyGluttons(){
  ...
  const {
    tokens, walletBalance, totalMinted, burned,
    loading, refreshing, degraded, error, refresh, refreshIds,
    rescan, address,
  } = useOwnedGluttons();
  const [selectedId, setSelectedId] = useState<number>();
  const [prey, setPrey] = useState('');
  const [now, setNow] = useState(() => Math.floor(Date.now() / 1000));
  const readSafe = !degraded && !p.rpcDegraded && p.synced;

  useEffect(() => { if (stage !== 'live' && stage !== 'syncing') router.replace('/'); }, [stage, router]);
  useEffect(() => { const ms=gameHour<=60?1000:3000;const timer = setInterval(() => setNow(Math.floor(Date.now()/1000)),ms); return () => clearInterval(timer); }, [gameHour]);
  ...
  return <><Header/><main className={`page-shell inventory-page phase-${p.currentPhase.toLowerCase().replace('_','-')}`}>
    ...
    {p.currentPhase==='LAST_SUPPER'&&!p.isSettled&&<Panel className="inventory-phase-lock"><b>LAST SUPPER ACTION LOCK</b><span>FEED · FAST · KEEP FRESH ARE PERMANENTLY CLOSED. POISON · FRESH · ROTTEN · LIVE DEVOUR · TRUCE REMAIN.</span></Panel>}
    {!address&&<Panel className="empty-state"><h2>CONNECT YOUR WALLET.</h2><p>Your positions come from the shared index of canonical ERC-721 ownership. No sample inventory is used.</p></Panel>}
    {address&&loading&&<Panel className="empty-state loading-grid"><h2>LOADING WALLET INVENTORY…</h2><p>Loading this wallet from the shared ownership index. The browser never scans the full NFT supply.</p></Panel>}
    {degraded&&<Panel className="error-state"><b>POSITION STATE STALE — GAMEPLAY ACTIONS PAUSED</b><p>Last confirmed ownership and token state remain visible, but survival transactions stay disabled until the shared index catches up.</p></Panel>}
    {error&&<Panel className="error-state"><b>INVENTORY READ DEGRADED</b><p>{error}</p><button onClick={rescan} className="ghost-btn">RETRY INVENTORY</button></Panel>}
    ...
    {tokens.length>0&&<>
      <div className="inventory-summary"><span>WALLET <b>{walletBalance.toLocaleString()}</b></span><span>OWNED INDEX <b>{tokens.length.toLocaleString()}</b></span><span>GAME SUPPLY <b>{totalMinted.toLocaleString()}</b></span><span>SURVIVORS* <b>{survivors.length}</b></span><span>FRESH* <b>{corpses.filter(t=>isGameplayFresh(t,now)).length}</b></span><span>ROTTEN* <b>{corpses.filter(t=>!isGameplayFresh(t,now)).length}</b></span><button onClick={refresh} disabled={refreshing}>{refreshing?'REFRESHING…':'REFRESH INVENTORY ↻'}</button><button onClick={rescan}>FORCE REFRESH</button></div>
      <div className="mt-4 grid gap-4 lg:grid-cols-[.62fr_1.38fr]">
        ...
        <div className="space-y-4">{selected?<SelectedPosition t={selected} all={survivors} now={now} phase={p.currentPhase} gameHour={gameHour} plagueUnlocked={plagueUnlocked} truceThreshold={Number(p.truceThreshold)} prey={prey} setPrey={setPrey} refreshIds={refreshIds} readSafe={readSafe}/>:<Panel className="empty-state"><h2>{p.isSettled?'NO LIVING POSITION IN THIS WALLET.':'NO LIVING POSITION SELECTED.'}</h2></Panel>}
          <Panel className="p-5"><div className="flex flex-wrap items-center justify-between gap-3"><Kicker>corpse inventory</Kicker><span className="text-[10px] text-[#ff5b2e]">THE DEAD BECOME FOOD.</span></div>{corpses.length?<div className="corpse-grid">{corpses.map(c=><CorpseCard key={c.id} corpse={c} survivors={survivors} now={now} gameHour={gameHour} bellAt={bellAt} phase={p.currentPhase} refreshIds={refreshIds} readSafe={readSafe}/>)}</div>:<p className="muted-copy">No Fresh or Rotten Gluttons owned by this wallet.</p>}</Panel>
        </div>
      </div>
    </>}
...
type PoisonTargetInfo={id:number;owner:string;visualState:number;expiry:number;isHungry:boolean;poisonProtectedUntil:number;finalBiteDeadline:number;fasting:boolean;updatedBlock:number};

function SelectedPosition({t,all,now,phase,gameHour,plagueUnlocked,truceThreshold,prey,setPrey,refreshIds,readSafe}:{t:GluttonToken;all:GluttonToken[];now:number;phase:string;gameHour:number;plagueUnlocked:boolean;truceThreshold:number;prey:string;setPrey:(x:string)=>void;refreshIds:(ids:number[])=>Promise<void>;readSafe:boolean}){
  const st=statusOf(t,now,gameHour);const rem=remaining(t,now);const gameplayOpen=phase!=='SETTLED'&&readSafe;const preLS=gameplayOpen&&phase!=='LAST_SUPPER';const rescue=t.fasting||t.finalBiteDeadline>now;const displayState=phase==='SETTLED'?'FINALIST':st;
  const canFeed=preLS&&(rem<=12*gameHour||rescue);const canFast=preLS&&rem<=12*gameHour&&!t.fasting;const canPoison=gameplayOpen&&!t.fasting&&t.finalBiteDeadline<=now&&rem>gameHour; // LOCK 06: poison stays live in LAST_SUPPER
  ...
  return <Panel className="selected-position p-5 md:p-6"><div className="flex items-center justify-between"><Kicker>selected position</Kicker><span className="text-xs text-zinc-500">GLUTTON #{String(t.id).padStart(4,'0')}</span></div><div className="selected-grid"><div><div className="selected-state"><div className="state-art"><GluttonArt t={t}/></div><div><strong className={phase==='SETTLED'?'text-emerald-400':tone[st]}>{displayState}</strong><small>{phase==='SETTLED'?'FINAL READ-ONLY POSITION':'CAN THIS GLUTTON ACT RIGHT NOW?'}</small></div><time className={phase!=='SETTLED'&&(st==='HUNGRY'||st==='FINAL BITE')?'timer-blink':''}>{phase==='SETTLED'?'CLOSED':st==='FINAL BITE'?gameClock(t.finalBiteDeadline-now,gameHour):gameClock(rem,gameHour)}</time></div>
    ...
    {devourPhase&&<div className="devour-panel"><div><b>DEVOUR THE LIVING</b><p>Burn one other living Glutton you own. The eater goes to 36H. The prey creates no corpse.</p></div><select value={prey} onChange={e=>setPrey(e.target.value)}><option value="">SELECT YOUR PREY</option>{preyOptions.map(x=><option key={x.id} value={x.id}>#{x.id}</option>)}</select><TxButton label="DEVOUR OWN GLUTTON" address={CONTRACTS.gameEngine} abi={GAME_ENGINE_ABI} functionName="liveDevour" args={[BigInt(t.id),BigInt(prey||0)]} disabled={!readSafe||!canDevour||!prey} className="danger-pulse" onConfirmed={()=>refreshIds([t.id,Number(prey)])}/></div>}
    {phase==='LAST_SUPPER'&&<TruceVoteCard tokenId={t.id} owner={t.owner} threshold={truceThreshold} readSafe={readSafe}/>} 
  </div></div></Panel>;
}

function TruceVoteCard({tokenId,owner,threshold,readSafe}:{tokenId:number;owner:string;threshold:number;readSafe:boolean}){
  const p=useAtomValue(protocolAtom);
  const [valid,setValid]=useState(false);
  const [syncing,setSyncing]=useState(false);
  const open=readSafe&&p.aliveCount>1n&&p.aliveCount<=BigInt(threshold||0);
  const readVote=async(fresh=false)=>{
    try{
      const r=await readApi<any>(`/api/read/token/${tokenId}`,{fresh,ttlMs:1800});
      const t=r.value.token as TokenSnapshot;
      const epoch=BigInt(r.value.protocol?.truceEpoch||'0');
      setValid(BigInt(t.voteEpoch||'0')===epoch&&String(t.voteOwner||'').toLowerCase()===owner.toLowerCase());
    }catch{/* preserve last confirmed vote */}
  };
  useEffect(()=>{void readVote()},[tokenId,owner,p.currentPhase,p.aliveCount]); // eslint-disable-line react-hooks/exhaustive-deps
  const confirmed=async()=>{setSyncing(true);forgetReadCache('/api/read/token');for(let i=0;i<8;i++){await new Promise(r=>setTimeout(r,500+i*250));await readVote(true);try{const r=await readApi<any>(`/api/read/token/${tokenId}`,{fresh:true,ttlMs:0});const t=r.value.token as TokenSnapshot;const epoch=BigInt(r.value.protocol?.truceEpoch||'0');if(BigInt(t.voteEpoch||'0')===epoch&&String(t.voteOwner||'').toLowerCase()===owner.toLowerCase()){setValid(true);break;}}catch{}}setSyncing(false)};
  return <div className={`truce-panel ${open?'open':'locked'}`}><div><b>THE TRUCE</b><p>{open?'RETIRE records this NFT’s current-owner vote. N/N surviving NFTs must vote in the same epoch. Clocks keep running.':`LOCKED UNTIL ${threshold} SURVIVORS OR FEWER.`}</p></div><TxButton label={!open?'TRUCE LOCKED':valid?'RETIRE VOTE CAST ✓':syncing?'INDEXING VOTE…':'RETIRE / VOTE TRUCE'} successLabel="VOTE CONFIRMED ONCHAIN ✓" address={CONTRACTS.gameEngine} abi={GAME_ENGINE_ABI} functionName="voteTruce" args={[BigInt(tokenId)]} disabled={!readSafe||!open||valid||syncing} onConfirmed={confirmed}/></div>;
}
...
function PoisonTargetControl({attacker,now,phase,gameHour,attackerCanPoison,onConfirmed}:{attacker:GluttonToken;now:number;phase:string;gameHour:number;attackerCanPoison:boolean;onConfirmed:(targetId:number)=>void}){
  ...
  const [poisonFlash,setPoisonFlash]=useState<{targetId:number;before:number;after?:number;shield?:number;finalBite?:number;wasFasting:boolean;indexing?:boolean}|null>(null);
  useEffect(()=>{setTarget('');setInfo(null);setReadError(null);setPoisonFlash(null)},[attacker.id]);
  useEffect(()=>{
    if(!target){setInfo(null);setReadError(null);setChecking(false);return;}
    const id=Number(target);
    if(!Number.isSafeInteger(id)||id<=0){setInfo(null);setReadError('INVALID TARGET ID');setChecking(false);return;}
    let dead=false;setChecking(true);setReadError(null);
    const timer=setTimeout(async()=>{try{const r=await readApi<any>(`/api/read/token/${id}`,{fresh:true,ttlMs:0});if(dead)return;const t=r.value.token as TokenSnapshot;const status=String(r.value.status||'UNKNOWN');if(t.burned||status==='CONSUMED'){setInfo(null);setReadError('TARGET CONSUMED / BURNED');return;}setInfo({id,owner:String(t.owner||''),visualState:status==='FRESH'?2:status==='ROTTEN'?3:1,expiry:Number(t.expiry||0),isHungry:status==='HUNGRY',poisonProtectedUntil:Number(t.poisonProtectedUntil||0),finalBiteDeadline:Number(t.finalBiteDeadline||0),fasting:Boolean(t.fasting),updatedBlock:Number(t.updatedBlock||0)});setReadError(null);}catch(e:any){if(!dead){setInfo(null);setReadError(e?.message||'TARGET STATE UNAVAILABLE')}}finally{if(!dead)setChecking(false)}},250);
  ...
  let targetStatus='ENTER TARGET ID',buttonLabel='POISON',statusClass='idle',detail='Manual token ID. No official victim list.';if(target&&checking){targetStatus='READING TARGET…';buttonLabel='CHECKING TARGET';statusClass='checking';detail='Reading the shared canonical state before the wallet opens.'}else if(target&&readError){targetStatus='TARGET UNAVAILABLE';buttonLabel='INVALID TARGET';statusClass='blocked';detail='The indexer cannot confirm a legal target. No transaction will be offered.'}else if(info){if(isSelf){targetStatus='SELF TARGET';buttonLabel='CANNOT POISON SELF';statusClass='blocked';detail='Choose another Glutton.'}else if(deadTarget){targetStatus='TARGET DEAD';buttonLabel='TARGET DEAD';statusClass='blocked';detail='Dead Gluttons cannot receive Poison.'}else if(finalBiteActive){targetStatus='FINAL BITE';buttonLabel='CANNOT POISON';statusClass='blocked';detail='This Glutton is already in Final Bite.'}else if(info.fasting){targetStatus='FASTING — VULNERABLE';buttonLabel='POISON';statusClass='vulnerable';detail='FAST drops the Poison shield. A successful hit triggers a 1H Final Bite.'}else if(protectedTarget){targetStatus='PROTECTED';buttonLabel='TARGET PROTECTED';statusClass='protected';detail='Shield is UP. Wait for it to fall; the life clock keeps running.'}else if(clockTooLow){targetStatus='CLOCK TOO LOW';buttonLabel='CANNOT POISON';statusClass='blocked';detail='Normal Poison requires the target to have more than 1H remaining.'}else{targetStatus='VULNERABLE';buttonLabel='POISON';statusClass='vulnerable';detail='Shield is DOWN. The contract remains the final authority when the wallet transaction executes.'}}
  if(!attackerCanPoison){if(phase==='SETTLED')buttonLabel='POISON CLOSED';else if(attacker.fasting)buttonLabel='FASTING CANNOT POISON';else if(attacker.finalBiteDeadline>now)buttonLabel='FINAL BITE';else if(remaining(attacker,now)<=gameHour)buttonLabel='ATTACKER CLOCK TOO LOW'}
  const handlePoisonConfirmed=async()=>{const targetId=info?.id||Number(target);if(!targetId)return;const before=targetRemain;const wasFasting=Boolean(info?.fasting);const beforeBlock=info?.updatedBlock||0;onConfirmed(targetId);setPoisonFlash({targetId,before,wasFasting,indexing:true});forgetReadCache('/api/read/token');let updated:PoisonTargetInfo|null=null;for(let i=0;i<10;i++){await new Promise(r=>setTimeout(r,550+i*220));try{const r=await readApi<any>(`/api/read/token/${targetId}`,{fresh:true,ttlMs:0});const t=r.value.token as TokenSnapshot;if(Number(t.updatedBlock||0)>beforeBlock){const status=String(r.value.status||'');updated={id:targetId,owner:String(t.owner||''),visualState:status==='FRESH'?2:status==='ROTTEN'?3:1,expiry:Number(t.expiry),isHungry:status==='HUNGRY',poisonProtectedUntil:Number(t.poisonProtectedUntil),finalBiteDeadline:Number(t.finalBiteDeadline),fasting:Boolean(t.fasting),updatedBlock:Number(t.updatedBlock)};break;}}catch{}}
    if(updated){const n=Math.floor(Date.now()/1000);setInfo(updated);setPoisonFlash({targetId,before,after:Math.max(0,updated.expiry-n),shield:Math.max(0,updated.poisonProtectedUntil-n),finalBite:Math.max(0,updated.finalBiteDeadline-n),wasFasting,indexing:false});}else setPoisonFlash({targetId,before,wasFasting,indexing:true});setTimeout(()=>setPoisonFlash(null),6500)};
  const canSubmit=attackerCanPoison&&validTarget&&!checking&&!readError;
  return <Action title="POISON" cost={`0.0004 ${NATIVE_SYMBOL}`} desc={attackerCanPoison?'Lock a target ID. Read its clock + shield before signing.':'Current attacker state cannot Poison.'}><div className="poison-lock">{poisonFlash&&<div className="poison-success" role="status"><small>{poisonFlash.indexing?'POISON CONFIRMED ONCHAIN':'POISON SUCCESSFUL'}</small><strong>#{String(poisonFlash.targetId).padStart(4,'0')} HIT</strong>{poisonFlash.indexing?<><span>READ MODEL CATCHING UP</span><b>THE TRANSACTION IS CONFIRMED. DISPLAY VALUES WILL UPDATE WHEN INDEXED.</b></>:poisonFlash.wasFasting?<><span>FINAL BITE TRIGGERED</span><time>{gameClock(poisonFlash.finalBite||gameHour,gameHour)}</time><b>FEED OR DIE.</b></>:<><span>TARGET CLOCK</span><time>{gameClock(poisonFlash.before,gameHour)} → {gameClock(poisonFlash.after||0,gameHour)}</time><span>POISON SHIELD</span><b>UP · {gameClock(poisonFlash.shield||0,gameHour)}</b></>}<em>YOUR CLOCK −1H</em></div>}<div className="poison-target-input"><input value={target} onChange={e=>setTarget(e.target.value.replace(/\D/g,''))} placeholder="TARGET TOKEN ID" inputMode="numeric"/><span>PUBLIC READ // NO TARGET LIST</span></div>{target&&<div className={`target-lock-card ${statusClass}`}><div className="target-lock-head"><div><small>TARGET LOCK</small><strong>#{String(info?.id||Number(target)||0).padStart(4,'0')}</strong></div><b>{targetStatus}</b></div>{info&&<div className="target-timers"><div><span>LIFE CLOCK</span><time className={!deadTarget&&targetRemain<=gameHour?'timer-blink':''}>{deadTarget?'DEAD':gameClock(targetRemain,gameHour)}</time></div><div><span>POISON SHIELD</span><time>{deadTarget?'N/A':info.fasting?'DOWN':shieldRemain>0?`UP · ${gameClock(shieldRemain,gameHour)}`:'DOWN'}</time></div></div>}<p>{detail}</p></div>}<TxButton label={buttonLabel} successLabel="POISON CONFIRMED ✓" address={CONTRACTS.gameEngine} abi={GAME_ENGINE_ABI} functionName="poison" args={[BigInt(attacker.id),BigInt(target||0)]} value={POISON_PRICE} disabled={!canSubmit} preflight onConfirmed={handlePoisonConfirmed}/></div></Action>;
}

function CorpseCard({corpse,survivors,now,gameHour,bellAt,phase,refreshIds,readSafe}:{corpse:GluttonToken;survivors:GluttonToken[];now:number;gameHour:number;bellAt:number;phase:string;refreshIds:(ids:number[])=>Promise<void>;readSafe:boolean}){
  const fresh=isGameplayFresh(corpse,now);const [eater,setEater]=useState('');const settled=phase==='SETTLED';const lastSupper=phase==='LAST_SUPPER';const effectivePowerEnd=bellAt>0&&corpse.poweredUntil>bellAt?bellAt:corpse.poweredUntil;const powered=effectivePowerEnd>now&&!lastSupper;const freshness=corpseFreshness(corpse,now,gameHour,bellAt);const eligible=survivors.filter(s=>{const r=remaining(s,now);return fresh?r<=12*gameHour:r<=6*gameHour});const refreshCorpse=()=>refreshIds([corpse.id]);
  return <div className={`corpse-card ${fresh?'fresh':'rotten'} ${settled?'readonly':''}`}><div className="corpse-head"><GluttonArt t={corpse}/><div><strong>#{String(corpse.id).padStart(4,'0')}</strong><span>{fresh?'FRESH CORPSE':'ROTTEN / EMERGENCY FOOD'}</span></div></div>{fresh&&<div className="freshness-panel"><div className="freshness-head"><span>FRESHNESS</span><b>{freshness.pct.toFixed(0)}%</b></div><div className="freshness-bar"><i style={{width:`${freshness.pct}%`}}/></div><div className="freshness-times"><div><span>ROTS IN</span><time>{gameClock(freshness.secondsToRot,gameHour)}</time></div><div><span>FRIDGE</span><time>{lastSupper?'DISABLED':powered?`ON · ${gameClock(freshness.fridgeLeft,gameHour)}`:'OFF'}</time></div></div></div>}
    <div className="corpse-status">{settled?<><b>GAME CLOSED</b><span>Final corpse state is read-only after settlement.</span></>:fresh?<>{lastSupper?<b>FRIDGE CLOSED AT THE BELL</b>:powered?<b>FRIDGE ACTIVE · {gameClock(freshness.fridgeLeft,gameHour)}</b>:<b>FRIDGE OFF</b>}<span>{lastSupper?'Fresh remains edible, but KEEP FRESH is permanently closed.':'KEEP FRESH slows rot 4× and never resets freshness. Any active power is capped by the Last Supper bell.'}</span></>:<><b>EMERGENCY FOOD</b><span>Rotten can be consumed by any eater at or below 6H, and it ADDS +2H (capped at 36H).</span></>}</div>
    {!settled&&<>{fresh&&!lastSupper&&<TxButton label={powered?'KEEP FRESH — ACTIVE':'KEEP FRESH'} successLabel="FRIDGE ACTIVATED ✓" address={CONTRACTS.gameEngine} abi={GAME_ENGINE_ABI} functionName="powerFridge" args={[BigInt(corpse.id)]} value={POWER_PRICE} disabled={!readSafe||powered} className="w-full" onConfirmed={refreshCorpse}/>}<div className="eat-explainer"><b>{fresh?'EAT THIS FRESH CORPSE':'EAT ROTTEN — EMERGENCY'}</b><span>{fresh?'Gives +12H to one Hungry Glutton you own. The corpse is permanently consumed.':'Adds +2H to the eater clock, capped at 36H. The corpse is permanently consumed.'}</span></div><div className="eat-row"><select value={eater} onChange={e=>setEater(e.target.value)}><option value="">SELECT {fresh?'HUNGRY ':''}EATER</option>{eligible.map(s=><option key={s.id} value={s.id}>#{s.id} · {gameClock(remaining(s,now),gameHour)}</option>)}</select><TxButton label={fresh?'EAT FRESH':'EAT ROTTEN'} successLabel="CORPSE CONSUMED ✓" address={CONTRACTS.gameEngine} abi={GAME_ENGINE_ABI} functionName="consumeCorpse" args={[BigInt(eater||0),BigInt(corpse.id)]} disabled={!readSafe||!eater} className="secondary-action" onConfirmed={()=>refreshIds([corpse.id,Number(eater)])}/></div></>}
  </div>;
}
